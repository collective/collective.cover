...
class IPersistentCoverTile(model.Schema):
    ...
    form.widget(
        css_class="collective.cover.tiles.configuration_widgets.cssclasswidget.CSSClassFieldWidget"
    )
...
class PersistentCoverTile(ESIPersistentTile):
    ...
    def remove_item(self, uuid):
        if not self.isAllowedToEdit():
            raise Unauthorized(_("You are not allowed to remove content of this tile"))
