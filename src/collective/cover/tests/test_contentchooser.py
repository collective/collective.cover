...
class ContentChooserTestCase(unittest.TestCase):
    ...
    def test_searches(self):
        ...
        document_match = tree.xpath(
            "//li[@data-content-type='Document']/a[@class='contenttype-document state-missing-value'][contains(@title, 'This document was created for testing purposes')][contains(@title, '/my-document')][@rel='1']"
        )
        self.assertFalse(document_match)
        image_match = tree.xpath(
            "//li[@data-content-type='Image']/a[@class='contenttype-image state-missing-value'][contains(@title, 'This image #2 was created for testing purposes')][contains(@title, '/my-image')][@rel='1']"
        )
    ...
    def test_asian_lang_searches(self):
        ...
        view = api.content.get_view(u"content-search", self.cover, self.request)
        self.portal["my-document"].setTitle(u"日本語のコンテンツを追加します。, " u"検索にかかるように設定します。")
        self.portal["my-document"].reindexObject()
